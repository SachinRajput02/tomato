...
const registerSeller = async (req, res) => {
  const { name,shopName, email, password,phone,ShopPic,address,fcmToken } = req.body;

  try {
    const existsShop = await sellerModel.findOne({ shopName });
    if (existsShop)
      return res.json({ success: false, message: "ShopName  already taken" });
