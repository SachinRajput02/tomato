...
const getSellerProfile = async (req,res)=>{
  try {
      let sellerData = await sellerModel.findById(req.body.sellerId);
      let name = await sellerData.name;
      let email = await sellerData.email;
      let phone = await sellerData.phone;
      let shopName = await sellerData.shopName;
      
      res.json({success:true,name,email,phone,shopName})
  } catch (error) {
      console.log(error);
      res.json({success:false,message:"Error to getting seller profile details"})
  }
}
...
export  {addToSellerCart,removeFromSellerCart,getsellerCart,getSellerProfile};